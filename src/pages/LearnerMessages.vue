      ...
      <div v-if="!selectedConversation" class="md-layout-item md-size-100">
        <md-card>
          <md-card-header data-background-color="green">
            ...
            <p class="category">Chat with your tutors</p>
          </md-card-header>
          <md-card-content>
            <div v-if="conversations.length === 0" class="md-body-1">
              No messages yet. Start a conversation with your tutor after they accept your request.
            </div>
            <div v-else>
              <md-list>
                <md-list-item
        ...
        <md-card>
          <md-card-header data-background-color="green">
            <div style="display: flex; justify-content: space-between; align-items: center; width: 100%">
              <div>
                <h4 class="title">{{ selectedConversation.tutorName }}</h4>
  ...
  methods: {
    loadConversations() {
      ...
      this.conversations = acceptedRequests.map((request) => {
        // Find the last message for this tutor
        const tutorMessages = allMessages.filter(
          (m) => m.tutorId === request.tutorId
        );
        const lastMessage = tutorMessages.length > 0
          ? tutorMessages[tutorMessages.length - 1]
          : null;
    ...
    loadChatMessages() {
      ...
      const allMessages = getLearnerMessages(this.currentUser.id);
      this.chatMessages = allMessages
        .filter(
          (m) =>
            m.tutorId === this.selectedConversation.tutorId &&
            ((m.studentId && m.studentId === this.currentUser.id) ||
             (m.studentName && m.studentName === this.currentUser.name))
        )
    ...
    sendMessage() {
      ...
      const message = {
        tutorId: this.selectedConversation.tutorId,
